  ...
  return (
    <div className="space-y-5">
      ...
      <section className="glass-card rounded-3xl p-5">
        <div className="overflow-x-auto rounded-2xl border border-(--line) bg-(--bg-surface)/85 p-2 md:overflow-x-hidden">
                ...
                <td>
                  ...
                  {(item.articleCount ?? 0) > 0 ? (
                    <span className="group relative inline-flex">
                      ...
                      <span className="pointer-events-none absolute -left-2 -top-9 z-20 whitespace-nowrap rounded-md border border-(--line) bg-(--bg-surface) px-2 py-1 text-xs text-(--ink) opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
                        Category is in use
                      </span>
                    </span>
                  ) : (
      ...
      {open ? (
        <CategoryDialog
          ...
          onSave={async (payload) => {
            if (editing) {
              await apiMutation<Category>(`/api/categories/${editing.id}`, "PATCH", payload);
        ...
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4">
          <div className="glass-card w-full max-w-md rounded-3xl p-4">
            ...
            <p className="mt-2 text-sm text-(--ink-soft)">
              Delete category <strong>{deletingCategory.name}</strong>? This action cannot be undone.
            </p>
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4">
      <div className="glass-card w-full max-w-md rounded-3xl p-4">
        ...
        <div className="mt-3 grid gap-2">
          <div>
            <FieldLabel text="Name" tip="Human-readable category name shown across the UI." />
          <input
        ...
        <div className="mt-4 flex justify-end gap-2">
          <button className="rounded-lg border border-(--line) px-3 py-2" onClick={onClose}>
            Cancel
          </button>
          <button
